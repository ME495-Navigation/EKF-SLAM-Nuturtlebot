/// \file circle.cpp
/// \brief This is a ROS2 node that publishes a twist message to the cmd_vel topic to make the robot move in a circle.
/// PARAMETERS:
///   frequency (int): The frequency of the control loop.
/// PUBLISHES:
///   cmd_vel (geometry_msgs::msg::Twist): The twist message to make the robot move in a circle.
/// SERVICES:
///   control (nuturtle_control::srv::Control): The service to set the velocity and radius of the circle.
///   reverse (std_srvs::srv::Empty): The service to reverse the direction of the circle.
///   stop (std_srvs::srv::Empty): The service to stop the circle.
...
public:
  ...
  : Node("circle")
  {
    ...
    // services
    control_srv_ =
      this->create_service<nuturtle_control::srv::Control>(
      "control",
