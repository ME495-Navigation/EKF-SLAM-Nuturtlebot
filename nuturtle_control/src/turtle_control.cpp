...
/// \brief This is a ROS2 node that publishes wheel commands and joint states.
/// PARAMETERS:
...
/// PUBLISHES:
///   wheel_cmd (nuturtlebot_msgs::msg::WheelCommands): The wheel commands message.
///   joint_states (sensor_msgs::msg::JointState): The joint states message.
/// SUBSCRIBES:
///   cmd_vel (geometry_msgs::msg::Twist): The twist message to control the robot.
///   sensor_data (nuturtlebot_msgs::msg::SensorData): The sensor data message.
...
private:
  ...
  void cmdvel_callback(const geometry_msgs::msg::Twist & t)
  {
    ...
    if (wheelcom.right_velocity > motor_cmd_max) {
      RCLCPP_ERROR_STREAM(
        get_logger(), "Right wheel velocity exceeds max limit! Setting to motor_cmd_max.");
      wheelcom.right_velocity = motor_cmd_max;
    }
    if (wheelcom.left_velocity > motor_cmd_max) {
      RCLCPP_ERROR_STREAM(
        get_logger(), "Left wheel velocity exceeds limit! Setting to motor_cmd_max.");
  ...
  void sensordata_callback(const nuturtlebot_msgs::msg::SensorData & s)
  {
    ...
    if (first) {
      ...
      // calculate dt
      auto dt = s.stamp.sec + s.stamp.nanosec * 1e-9 - joints.header.stamp.sec -
        joints.header.stamp.nanosec * 1e-9;
      joints.header.stamp = s.stamp;
...
public:
  ...
  : Node("turtle_control")
  {
    ...
    sensordata_sub = create_subscription<nuturtlebot_msgs::msg::SensorData>(
      "sensor_data", 10, std::bind(
        &TurtleControl::sensordata_callback, this,
        std::placeholders::_1));
  }
};
